...
setgame.factory('engine', ['deck', 'card',
  function(deck, card) {
    ...
    function isUnique(cards) {
      for (var i = 0; i < cards.length - 1; i++) {
        for (var j = i + 1; j < cards.length; j++) {
          ...
          var isSame = cards[i].color === cards[j].color && cards[i].fill === cards[j].fill && cards[i].shape === cards[j].shape && cards[i].number === cards[j].number
